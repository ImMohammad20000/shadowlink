...
LINUX_XRAY_DIR="$SCRIPT_DIR/linux"
...
resolve_xray_bin() {
  ...
  if [[ -n $arch_dir ]]; then
    xray_bin="$LINUX_XRAY_DIR/$arch_dir/xray"
    if [[ -f $xray_bin ]]; then
  ...
  printf '%b\n' "${RED}Error: Unix xray binary not found. Expected ${LINUX_XRAY_DIR}/${arch_dir:-<arch>}/xray, client/xray, or xray on PATH; alternatively set XRAY_BIN.${NC}"
  exit 1
}
